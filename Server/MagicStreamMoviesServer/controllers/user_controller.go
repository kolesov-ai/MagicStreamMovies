...
func LogoutHandler(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		//	false, //Use true with production with HTTPS
		//	true, //HttpOnly
		//	)
...
func RefreshTokenHandler(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			fmt.Println("Refresh Token Handler Error:", err.Error())
			c.JSON(http.StatusUnauthorized, gin.H{"error": "Enable to retrieve refresh token", "details": err.Error()})
			return
		}
		...
		c.SetCookie("refresh_token", newRefreshToken, 86400, "/", "localhost", true, true)
		c.JSON(http.StatusOK, gin.H{"message": "Refresh Token Handler successfully"})
	}
}
